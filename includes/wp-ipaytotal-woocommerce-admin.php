...
class wowp_iptwpg_ipaytotal extends WC_Payment_Gateway_CC {
	...
	public function process_payment( $order_id ) {
        ...
        $data = array(
            ...
            'state'         => empty($billing_state) ? $billing_city : $billing_state, // if your country US then use only 2 letter state code.
            'city'          => $billing_city,
		...
		if ($this->ipt_test_mode == 'yes') {
			$environment_url = "https://ipaytotal.solutions/api/test/transaction";
		} else {
		...
		if ( $response_body['status'] == '3d_redirect' ) {
		    ...
		    return array(
		        'result' => 'success',
		        'redirect' => $response_body['redirect_3ds_url']
		    );

		} elseif ( $response_body['status'] == 'success' ) {
